package com.derik.library.utils.bar;
...
public class StatusBarUtil {
    ...
    public static void setStatusBar(Activity activity, int statusBgColor, int navigationBgColor, boolean statusTextDart) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {//5.0 above
            activity.getWindow().setStatusBarColor(statusBgColor);
            activity.getWindow().setNavigationBarColor(navigationBgColor);
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {//4.4 to 5.0
            WindowManager.LayoutParams localLayoutParams = activity.getWindow().getAttributes();
            localLayoutParams.flags = (WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS | localLayoutParams.flags);
        }
        setStatusTextColor(activity, statusTextDart);
    }
